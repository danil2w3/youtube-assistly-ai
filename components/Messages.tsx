...
function Messages({ messages, chatBotName}: {
  ...
  return (
    <div className="flex-1 flex-col overflow-y-auto space-y-10 py-10 
    px-5 bg-white rounded-lg">
        {messages.map((message) => {
            const isSander = message.sender !== 'user';
            return (
            <div
             key={message.id}
             className={`chat ${isSander ? 'chat-start' : 'chat-end'}
            relative`}
            >
                {isReviewsPage && (
                ...
                <div className={`chat-image avatar w-10 ${!isSander && 
                    "-mr-4"}`}>
                    {isSander ? (
                        <Avatar
                        seed={chatBotName}
                    ...
                    components={{
                        ...
                        p: ({ node, ...props }) => (
                            <p 
                              ...
                              message.content === "Tainking..." && "animate-pulse"
                            } ${isSander ? "text-white" : "text-gray-700"}`}
                            />
                        ),
